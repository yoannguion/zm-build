...
sub Clone($$)
{
   ...
   my $repo_tag_csv    = $CFG{GIT_OVERRIDES}->{"$repo_name.tag"} || $repo_details->{tag} || $CFG{GIT_DEFAULT_TAG} if ( $CFG{GIT_OVERRIDES}->{"$repo_name.tag"} || ( !$repo_details->{branch} && !$CFG{GIT_OVERRIDES}->{"$repo_name.branch"}) );
