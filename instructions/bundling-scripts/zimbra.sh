...
CreateRhelPackage()
{
  ...
  zimbra=( base perl perl-base apache-base memcached-base core-components apache-components dnscache-components ldap-components mta-components os-requirements proxy-components spell-base spell-components  )
...
sudo -E bash -c 'cat >  /etc/yum.repos.d/zimbra-build.repo << EOF
[zimbra-build]
...
enabled=1
EOF'
cat /etc/yum.repos.d/zimbra-build.repo
}
############################################################################
main "$@"